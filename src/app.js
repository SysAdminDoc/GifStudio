        ...
        class GIFEditor {
            ...
            reportStorageError(context, error) {
                ...
                this.showToast(
                    quota
                        ? 'Local recovery storage is full. Export your work, then dismiss an older recovery.'
            ...
            async checkSessionRecovery() {
                ...
                try {
                    ...
                    if (!session) return;
                    const migrated = this.migrateSession(session);
                    ...
                    this.showRecoveryBanner(migrated, timeAgo);
                } catch (e) {
                    if (e?.name === 'VersionError') {
                        ...
                        await this.clearSavedSession({ reportErrors: false });
            ...
            showRecoveryBanner(session, timeAgo) {
            ...
            async clearSavedSession({ reportErrors = true } = {}) {
                let db;
                try {
                    db = await this.openDB();
            ...
            updateSizeEstimate() {
                ...
                if (!el || this.frames.length === 0) return;
                ...
                el.textContent = `~${this.formatBytes(estimated)} · ${this.formatDuration(outputDuration)}`;

                const limits = [
                    { name: 'Discord', max: 8 * 1024 * 1024 },
                    { name: 'Slack', max: 8 * 1024 * 1024 },
                    { name: 'Twitter', max: 15 * 1024 * 1024 }
                ];
                badges.innerHTML = limits.map(l => {
                    const ok = estimated <= l.max;
                    const color = ok ? 'var(--success)' : 'var(--danger)';
                    return `<span style="color:${color}">${ok ? '✓' : '✗'} ${l.name}</span>`;
                }).join('  ');
            }
