    // gifenc core — PNN quantizer, LZW encoder, GIF89a writer
    ...
    const gifenc = { GIFEncoder: ct, quantize: H, applyPalette: nt, prequantize: et, nearestColorIndex: W };

    class GifEncoder {
        constructor(width, height, opts = {}) {
            this.width = width;
            this.height = height;
            this.repeat = opts.repeat != null ? opts.repeat : 0;
            this.maxColors = Math.min(256, Math.max(2, opts.maxColors || 256));
            this.quality = Math.min(30, Math.max(1, parseInt(opts.quality, 10) || 10));
        ...
        addFrame(canvas, delay) {
            ...
            this.frames.push({ data: imageData.data, delay: delay });
        }

        abort() { this.aborted = true; }

        getPrequantizeRoundRGB() {
            if (this.quality <= 5) return 1;
            if (this.quality <= 15) return 3;
            if (this.quality <= 25) return 6;
            return 10;
        }

        applyDitheredPalette(data, palette, width, height) {
            const pixelCount = width * height;
            const work = new Float32Array(pixelCount * 3);
            const indices = new Uint8Array(pixelCount);
            const cache = new Map();

            for (let src = 0, dst = 0; src < data.length && dst < work.length; src += 4, dst += 3) {
                work[dst] = data[src];
                work[dst + 1] = data[src + 1];
                work[dst + 2] = data[src + 2];
            }
            ...
            const diffuse = (base, errR, errG, errB, factor) => {
                work[base] += errR * factor;
                work[base + 1] += errG * factor;
                work[base + 2] += errB * factor;
            };
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    const r = clampByte(work[base]);
                    const g = clampByte(work[base + 1]);
                    const b = clampByte(work[base + 2]);
                    const paletteIndex = nearest(r, g, b);
                    ...
                    if (x + 1 < width) diffuse(base + 3, errR, errG, errB, 7 / 16);
                    if (y + 1 < height) {
                        const nextRow = base + width * 3;
                        if (x > 0) diffuse(nextRow - 3, errR, errG, errB, 3 / 16);
                        diffuse(nextRow, errR, errG, errB, 5 / 16);
                        if (x + 1 < width) diffuse(nextRow + 3, errR, errG, errB, 1 / 16);
                    }
                }
            }
        ...
        render(onProgress) {
            return new Promise((resolve, reject) => {
                ...
                // Full-canvas frames are intentional. Local-frame deltas require
                // look-ahead disposal handling to clear opaque pixels back to
                // transparency; the prior encoder did not preserve that case.
                ...
                const processFrame = () => {
                    ...
                    const rgba = new Uint8Array(frame.data.buffer.slice(0));
                    const roundRGB = this.getPrequantizeRoundRGB();
                    if (roundRGB > 1) {
                        gifenc.prequantize(rgba, { roundRGB, roundAlpha: 1 });
                    }
                    const palette = gifenc.quantize(rgba, maxColors);
                    const indices = this.dither === 'FloydSteinberg'
                        ? this.applyDitheredPalette(rgba, palette, w, h)
                        : gifenc.applyPalette(rgba, palette);

                    enc.writeFrame(indices, w, h, {
                        palette: palette,
                        delay: frame.delay,
                        repeat: frameIdx === 0 ? this.repeat : undefined,
                        dispose: frameIdx === 0 ? -1 : 2
                    });

                    frameIdx++;
                    if (onProgress) {
                        onProgress(frameIdx / frames.length, {
